...
if(data.token.toString() === 'host'){
  await db.collection(data.gameCode).doc("hosttoken").get()
 .then((doc) => {
    if (!doc.exists) {
      throw new functions.https.HttpsError("invalid-argument", "User doesn't exist in db!");
    } else {
      registrationToken = /*doc.data().token*/'crMLWq8nTDGfl24QJ2uR8B:APA91bER1cskVgBf-F_QMsP5R2Sd_yYX7dTdMVZwgJx1e2Cy59FfzQjS2dqf3TTewHJCKxJVlZ8qbraZ5acsiQLE0LJgtyiQoo4DVj-0jJmvEA9mcOsQqXLezq0xclxVhPLZ4f0jPxJ2'
      return Promise
    }
  });
}
